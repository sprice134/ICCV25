...
def main():
    ...
    for config in model_configs:
        ...
        for img_path in image_paths:
            ...
            # -------------------------------------------------------
            # 5-B1) YOLO+SAM => original SAM config
            # -------------------------------------------------------
            ...
            # -------------------------------------------------------
            # 5-B2) YOLO+SAM => DualSight (3 points, border=5, Hill Climbing)
            # -------------------------------------------------------
            dualsight_masks_list = run_sam_inference(
                predictor=sam_predictor,
                loop_image=loop_image,
                listOfPolygons=listOfPolygons,
                listOfBoxes=listOfBoxes,
                ...
                image_width=pil_image.width,
                image_height=pil_image.height,
                num_points=3,                 # 3 points
                dropout_percentage=0,
                ignore_border_percentage=5,   # 5% border
                algorithm="Hill Climbing",    # Use "Hill Climbing"
                use_mask_input=False,
                box_expansion_rate=0.0
            )
            dualsight_16bit_path = os.path.join(
                output_dir, f"{image_name.replace('.jpg', f'_{model_name}_DualSight_16bit.png')}"
            )
            combine_masks_16bit(dualsight_masks_list, dualsight_16bit_path)

            # -------------------------------------------------------
            # 5-B3) YOLO+SAM => ZFR (1 point, no border, Hill Climbing)
            # -------------------------------------------------------
            zfr_masks_list = run_sam_inference(
                predictor=sam_predictor,
                loop_image=loop_image,
                listOfPolygons=listOfPolygons,
                listOfBoxes=listOfBoxes,
                ...
                image_width=pil_image.width,
                image_height=pil_image.height,
                num_points=1,                 # Only 1 point
                dropout_percentage=0,
                ignore_border_percentage=0,   # No border
                algorithm="Hill Climbing",    # Use "Hill Climbing"
                use_mask_input=False,
                box_expansion_rate=0.0
            )
            zfr_16bit_path = os.path.join(
                output_dir, f"{image_name.replace('.jpg', f'_{model_name}_ZFR_16bit.png')}"
            )
            combine_masks_16bit(zfr_masks_list, zfr_16bit_path)
            ...
            try:
                ...
                visualize_segmentations(
                    image_path=img_path,
                    mask_path=dualsight_16bit_path,
                    output_dir=output_dir,
                    title=f"{model_name} + DualSight"
                )
                visualize_segmentations(
                    image_path=img_path,
                    mask_path=zfr_16bit_path,
                    output_dir=output_dir,
                    title=f"{model_name} + ZFR"
                )
                visualize_segmentations(
                    image_path=img_path,
            ...
            # (3) DualSight
            gt_data, pred_data_dual = generate_coco_annotations_from_multi_instance_masks_16bit(
                gt_mask_path, dualsight_16bit_path, img_path
            )
            dual_segm_metrics = evaluate_coco_metrics(gt_data, pred_data_dual, iou_type="segm", max_dets=350)
            dual_bbox_metrics = evaluate_coco_metrics(gt_data, pred_data_dual, iou_type="bbox", max_dets=350)
            dual_precision, dual_recall = compute_pixel_precision_recall(gt_mask_path, dualsight_16bit_path)

            # (4) ZFR
            gt_data, pred_data_zfr = generate_coco_annotations_from_multi_instance_masks_16bit(
                gt_mask_path, zfr_16bit_path, img_path
            )
            zfr_segm_metrics = evaluate_coco_metrics(gt_data, pred_data_zfr, iou_type="segm", max_dets=350)
            zfr_bbox_metrics = evaluate_coco_metrics(gt_data, pred_data_zfr, iou_type="bbox", max_dets=350)
            zfr_precision, zfr_recall = compute_pixel_precision_recall(gt_mask_path, zfr_16bit_path)
            ...
            seg_small_precision, seg_small_recall = compute_pixel_precision_recall(
                gt_mask_path, segrefiner_small_16bit_path
            )
            ...
            seg_large_precision, seg_large_recall = compute_pixel_precision_recall(
                gt_mask_path, segrefiner_large_16bit_path
            )
            ...
            results.append({
                ...
                # DualSight bounding boxes
                "dualsight_box_AP@50": safe_get(dual_bbox_metrics, "AP@50"),
                "dualsight_box_AP@75": safe_get(dual_bbox_metrics, "AP@75"),
                "dualsight_box_AP@95": safe_get(dual_bbox_metrics, "AP@95"),
                "dualsight_box_AP@50:95": safe_get(dual_bbox_metrics, "AP@50:95"),
                "dualsight_box_AR@50": safe_get(dual_bbox_metrics, "AR@50"),
                "dualsight_box_AR@75": safe_get(dual_bbox_metrics, "AR@75"),
                "dualsight_box_AR@95": safe_get(dual_bbox_metrics, "AR@95"),
                "dualsight_box_AR@50:95": safe_get(dual_bbox_metrics, "AR@50:95"),

                # DualSight masks
                "dualsight_mask_AP@50": safe_get(dual_segm_metrics, "AP@50"),
                "dualsight_mask_AP@75": safe_get(dual_segm_metrics, "AP@75"),
                "dualsight_mask_AP@95": safe_get(dual_segm_metrics, "AP@95"),
                "dualsight_mask_AP@50:95": safe_get(dual_segm_metrics, "AP@50:95"),
                "dualsight_mask_AR@50": safe_get(dual_segm_metrics, "AR@50"),
                "dualsight_mask_AR@75": safe_get(dual_segm_metrics, "AR@75"),
                "dualsight_mask_AR@95": safe_get(dual_segm_metrics, "AR@95"),
                "dualsight_mask_AR@50:95": safe_get(dual_segm_metrics, "AR@50:95"),

                # DualSight Aggregate
                "dualsight_precision": dual_precision,
                "dualsight_recall": dual_recall,

                # ZFR bounding boxes
                "zfr_box_AP@50": safe_get(zfr_bbox_metrics, "AP@50"),
                "zfr_box_AP@75": safe_get(zfr_bbox_metrics, "AP@75"),
                "zfr_box_AP@95": safe_get(zfr_bbox_metrics, "AP@95"),
                "zfr_box_AP@50:95": safe_get(zfr_bbox_metrics, "AP@50:95"),
                "zfr_box_AR@50": safe_get(zfr_bbox_metrics, "AR@50"),
                "zfr_box_AR@75": safe_get(zfr_bbox_metrics, "AR@75"),
                "zfr_box_AR@95": safe_get(zfr_bbox_metrics, "AR@95"),
                "zfr_box_AR@50:95": safe_get(zfr_bbox_metrics, "AR@50:95"),

                # ZFR masks
                "zfr_mask_AP@50": safe_get(zfr_segm_metrics, "AP@50"),
                "zfr_mask_AP@75": safe_get(zfr_segm_metrics, "AP@75"),
                "zfr_mask_AP@95": safe_get(zfr_segm_metrics, "AP@95"),
                "zfr_mask_AP@50:95": safe_get(zfr_segm_metrics, "AP@50:95"),
                "zfr_mask_AR@50": safe_get(zfr_segm_metrics, "AR@50"),
                "zfr_mask_AR@75": safe_get(zfr_segm_metrics, "AR@75"),
                "zfr_mask_AR@95": safe_get(zfr_segm_metrics, "AR@95"),
                "zfr_mask_AR@50:95": safe_get(zfr_segm_metrics, "AR@50:95"),

                # ZFR Aggregate
                "zfr_precision": zfr_precision,
                "zfr_recall": zfr_recall,
