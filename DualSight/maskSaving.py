...
def main():
    ...
    # containing keys "boxes", "polygons", and "masks".
    ...
    # -------------------------------------------------------------------------
    # Save each individual refined mask as its own black-and-white image
    # -------------------------------------------------------------------------
    ...
    for idx, mask in enumerate(refined_masks):
        ...
        # If mask is binary (0 and 1) then multiply to get 0 and 255
        if mask.max() <= 1:
            mask = mask * 255

        mask_output_path = os.path.join(
            output_dir,
            f"{os.path.splitext(image_filename)[0]}_mask_{idx}_{unique_id}.png"
        )
        Image.fromarray(mask).save(mask_output_path)
        print(f"Saved individual mask {idx} to: {mask_output_path}")
