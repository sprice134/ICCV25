...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run experiments with optional filtering on DOI configurations."
    )
    parser.add_argument("--BoxInclusion", nargs="+", type=str,
                        help="Filter by BoxInclusion (e.g., True or False).", default=None)
    parser.add_argument("--MaskInclusion", nargs="+", type=str,
                        help="Filter by MaskInclusion (e.g., True or False).", default=None)
    parser.add_argument("--BoundingBoxDistortion", nargs="+", type=str,
                        help="Filter by BoundingBoxDistortion (e.g., '100%').", default=None)
    parser.add_argument("--Model", nargs="+", type=str,
                        help="Filter by Model (e.g., 'YOLOv8 Nano + Sam').", default=None)
    parser.add_argument("--NumberOfPOIs", nargs="+", type=int,
                        help="Filter by NumberOfPOIs (e.g., 1).", default=None)
    parser.add_argument("--ID", nargs="+", type=int,
                        help="Filter by ID.", default=None)
    args = parser.parse_args()

    # Build the experiment_filters dictionary from command-line arguments
    experiment_filters = {}
    if args.BoxInclusion is not None:
        # Convert string inputs to booleans (case-insensitive)
        experiment_filters["BoxInclusion"] = [True if x.lower() == "true" else False for x in args.BoxInclusion]
    if args.MaskInclusion is not None:
        experiment_filters["MaskInclusion"] = [True if x.lower() == "true" else False for x in args.MaskInclusion]
    if args.BoundingBoxDistortion is not None:
        experiment_filters["BoundingBoxDistortion"] = args.BoundingBoxDistortion
    if args.Model is not None:
        experiment_filters["Model"] = args.Model
    if args.NumberOfPOIs is not None:
        experiment_filters["NumberOfPOIs"] = args.NumberOfPOIs
    if args.ID is not None:
        experiment_filters["ID"] = args.ID

    if experiment_filters:
        print("[INFO] Using the following filters:", experiment_filters)
    else:
        print("[INFO] No filters provided. Running experiments for all configurations.")
