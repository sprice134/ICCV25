...
def refine_and_evaluate(
    ...
    2) For each image:
       ...
       - Refines the predicted masks using segmentation_refinement.
       - Binarizes the refined masks.
       - Evaluates them via COCO metrics against a ground truth mask located in an 'annotations' subdirectory.
    3) Returns image-level and aggregate metrics.
    ...
    Returns:
        dict: A dictionary containing aggregate metrics and per-image results.
    """
    ...
    for image_name, preds in inference_data.items():
        ...
        # e.g. if image_name = 'myImage.jpg', the GT path might be 'myImage_mask.png'
        ...
        # Evaluate with COCO metrics (iou_type="segm")
        try:
