...
OUTPUT_CSV="metrics/powder_cascadePSP_v3.csv"
OUTPUT_INDIVIDUAL_CSV="metrics/powder_cascadePSP_v3_individual.csv"
...
run_cascadePSP() {
    ...
    python cascadePSP_v3.py \
        --inference_pickle_path "$pkl_path" \
        --images_dir "$IMAGES_DIR" \
        --output_dir "$OUTPUT_DIR" \
        --run_name "$run_name" \
        --output_csv "$OUTPUT_CSV" \
        --instance_csv "$OUTPUT_INDIVIDUAL_CSV"
