...
def evaluate_models(inference_pickle_path, images_dir, output_csv, run_name):
    ...
    for image_id, (image_name, preds) in enumerate(inference_data.items(), start=1):
        ...
        # Evaluate segmentation metrics and box metrics
        try:
            segm_metrics = evaluate_coco_metrics(
                gt_data, pred_data_coco, iou_type="segm", max_dets=200
            )
            box_metrics = evaluate_coco_metrics(
                gt_data, pred_data_coco, iou_type="bbox", max_dets=200
            )
        except Exception as e:
        ...
        # Combine the segmentation and box metrics
        combined_metrics = {}
        for k, v in segm_metrics.items():
            combined_metrics[f"refined_mask_{k}"] = v
        for k, v in box_metrics.items():
            combined_metrics[f"refined_box_{k}"] = v

        metrics_accumulators.append(combined_metrics)

        print(f"[INFO] Evaluated {image_name}:")
        print(f"   Mask metrics: AP@50={segm_metrics.get('AP@50', 'N/A')}, AP@75={segm_metrics.get('AP@75', 'N/A')}, AP@95={segm_metrics.get('AP@95', 'N/A')}, AP@50:95={segm_metrics.get('AP@50:95', 'N/A')}")
        print(f"   Box metrics:  AP@50={box_metrics.get('AP@50', 'N/A')}, AP@75={box_metrics.get('AP@75', 'N/A')}, AP@95={box_metrics.get('AP@95', 'N/A')}, AP@50:95={box_metrics.get('AP@50:95', 'N/A')}")
