...
from sam_helper import load_sam_model  # and run_sam_inference if you use that—but here we include our own
...
# =============================================================================
#  Utility functions (including combine_masks_16bit, mask_to_coco_format, etc.)
# =============================================================================
...
def load_image(image_path):
    ...
    # Check if the image is actually grayscale (some images might be misinterpreted)
...
def generate_coco_annotations_from_multi_instance_masks_16bit(gt_mask_path, pred_mask_path, image_id=1, category_id=1):
    ...
    gt_annotations = []
    pred_annotations = []
    for ann_id, obj_id in enumerate(gt_ids, start=1):
        bin_mask = (gt_mask == obj_id).astype(np.uint8)
        gt_annotations.append(mask_to_coco_format(bin_mask, image_id, category_id, ann_id, score=1.0))
    for ann_id, obj_id in enumerate(pred_ids, start=1):
        bin_mask = (pred_mask == obj_id).astype(np.uint8)
        pred_annotations.append(mask_to_coco_format(bin_mask, image_id, category_id, ann_id, score=1.0))
    gt_data = {
        "images": [{
            "id": image_id,
...
def write_metrics_to_csv(run_name, metrics, output_csv_path):
    ...
    fieldnames = ['Run_Name'] + sorted(metrics.keys())
...
# =============================================================================
# (Below we incorporate logic from your second file to standardize point selection.)
...
from itertools import combinations  # Needed for naiveMaximization
...
def select_point_placement(mask, num_points, dropout_percentage=0, ignore_border_percentage=0, algorithm="Voronoi", select_perimeter=False):
    ...
    # No check against num_points^2; only skip if area==0.
...
# =============================================================================
# (We reuse load_sam_model from above.)
...
def run_sam_inference(sam_predictor, loop_image, listOfPolygons, listOfBoxes, listOfMasks,
    ...
    for idx in range(len(listOfPolygons)):
        ...
        if hasattr(mask, 'cpu'):
            mask = mask.cpu().numpy()
        # Always try to process a mask unless it is completely empty.
        if np.sum(mask) == 0:
        ...
        if algorithm in ["Hill Climbing"] and num_points != 1:
            ...
            try:
                ...
                print(f"[ERROR] selecting points: {e} (mask sum: {np.sum(mask)})")
                continue
        # Adjust mask area (if desired)
        mask = adjust_mask_area(mask, mask_expansion_rate * 100)  # treat mask_expansion_rate as a fraction
...
# =============================================================================
#  3) Main Inference and Evaluation Routine
# =============================================================================
...
def sam_inference_and_evaluate(inference_pickle_path, images_dir, output_dir,
    ...
    per_image_results = []
    metrics_accumulators = {"SAM_AP@50": [], "SAM_AP@75": [], "SAM_AP@95": [], "SAM_AP@50:95": [],
                             "SAM_AR@50": [], "SAM_AR@75": [], "SAM_AR@95": [], "SAM_AR@50:95": []}
    missing_masks = []
    for image_name, image_data in inference_data.items():
        ...
        try:
            ...
            loop_image = np.array(pil_image)[:, :, ::-1].copy()  # Convert RGB to BGR for consistency with OpenCV
        except Exception as e:
        ...
        # Create the base mask (even though we won't use base scores here)
        ...
        try:
            gt_data, pred_annotations = generate_coco_annotations_from_multi_instance_masks_16bit(gt_mask_path, sam_mask_path)
    ...
    summary = {"missing_masks": missing_masks, "aggregate_metrics": aggregate_metrics, "per_image_results": per_image_results}
    return summary
...
def parse_arguments():
    ...
    # Now require explicit boolean strings for use_box_input and use_mask_input.
